...
function ProductModal({ db }) {
    const navigate = useNavigate();
    const { id } = useParams();
    const list = db.filter(item => item.id.match(RegExp(id, "i")));

    return (
        <div className="modal d-block fade-in" tabIndex="-1">
            <div className="modal-dialog modal-xl modal-dialog-centered">
                {list.map(item => (
                    <div className="modal-content" key={item.id}>
                        <div className={`modal-header ${item.category.match(RegExp("best", "i")) ? "bg-featured" : `${item.sale > 0 ? "bg-success-subtle" : `${item.category.match(RegExp("new", "i")) ? "bg-danger-subtle" : "bg-warning-subtle"}`}`}`}>
                        ...
                        <div className="modal-body">
                            <div className="row">
                                ...
                                <div className="col-lg-6 mt-5 mt-lg-0 pt-4 pt-lg-0 mb-3">
                                    ...
                                    <div className="">{item.description.d3}</div>
                                </div>
                            </div>
                            <div className="mt-xl-5 pt-xl-4 container px-lg-5 mb-3">
                                <table className="table table-warning table-striped table-hover table-borderless rounded overflow-hidden">
                                    <thead>
                                        <tr>
                                            <th className="bg-danger-subtle text-center" scope="col" colspan="2">Product Info</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                        ...
                        <div className={`modal-footer ${item.category.match(RegExp("best", "i")) ? "bg-featured" : `${item.sale > 0 ? "bg-success-subtle" : `${item.category.match(RegExp("new", "i")) ? "bg-danger-subtle" : "bg-warning-subtle"}`}`}`}>
