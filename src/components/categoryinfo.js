...
function CategoryInfo({ db, cat }) {
    let { id } = useParams();
    const location = useLocation();
    const list = db.filter(item => item.category.match(RegExp(id, "i")))
    const category = cat.filter(item => item.link.match(RegExp(id, "i")))

    return (
        <div className="container-fluid mt-3 mb-5">
            <div className="h2 container">Category: {category.map(item => (item.name))}
                <hr />
            </div>
            <div className="mx-3">
